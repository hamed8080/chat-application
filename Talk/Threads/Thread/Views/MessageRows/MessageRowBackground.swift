...
struct MessageRowBackground: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        Path { path in
            ...
            let roundeedRect = CGRect(x: 0, y: 0, width: rect.width - MessageRowViewModel.tailSize.width, height: rect.height)
            path.addRoundedRect(in: roundeedRect, cornerSize: .init(width: 12, height: 12))
            if showTail {
                path.move(to: .init(x: roundeedRect.width, y: rect.height - MessageRowViewModel.tailSize.height))
                path.addQuadCurve(
                    to: .init(x: rect.width - (roundedCorner / 2), y: rect.height - roundedCorner),
                    control: .init(x: roundeedRect.width, y: rect.height - (MessageRowViewModel.tailSize.height / 2))
                )
                path.addArc(
                ...
                path.addLine(to: .init(x: roundeedRect.width - (MessageRowViewModel.tailSize.width * 2), y: rect.height))
            }
        }
    }
}
