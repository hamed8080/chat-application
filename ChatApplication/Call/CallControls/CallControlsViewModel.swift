...
class CallControlsViewModel: ObservableObject {
    ...
    private(set) var callId: Int?

    private(set) var connectionStatusCancelable: AnyCancellable?
    private(set) var startCallCancelable: AnyCancellable?
    ...
    init() {
        ...
        startCallCancelable = NotificationCenter.default.publisher(for: CALL_EVENT_NAME)
            ...
            .sink { event in
                ...
                if case .callStarted(let event) = event, let callId = event.callId {
                    self.callId = callId
                }
            }
    }
    ...
    func startRecording() {
        ...
        Chat.sharedInstance.startRecording(.init(subjectId: callId)) { [weak self] participant, _, _ in
    ...
    func stopRecording() {
        guard let callId = callId else { return }
        Chat.sharedInstance.stopRecording(.init(subjectId: callId)) { [weak self] participant, _, _ in
            if let _ = participant, let self = self {
