...
struct ThreadsModel {
    ...
    mutating func addTypingThread(_ event: SystemEventTypes) -> Bool {
        if case let .SYSTEM_MESSAGE(_, _, id) = event, typingThreadIds.contains(where: { $0 == id }) == false {
    ...
    mutating func removeTypingThread(_ event: SystemEventTypes) {
        if case let .SYSTEM_MESSAGE(_, _, id) = event, let index = typingThreadIds.firstIndex(where: { $0 == id }) {
            threadsTyping.remove(at: index)
        }
    }

    var typingThreadIds: [Int?] {
        threadsTyping.map { event -> Int? in
            guard case let .SYSTEM_MESSAGE(_, _, id) = event else { return nil }
            return id
        }
    }
