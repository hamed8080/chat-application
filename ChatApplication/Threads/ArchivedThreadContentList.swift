//
//  ArchivedThreadContentList.swift
//  ChatApplication
//
...
struct ArchivedThreadContentList:View {
        ...
        ZStack{
            VStack(spacing:0){
                List {
                    ...
                    ForEach(viewModel.model.threads.filter({$0.isArchive == true}) , id:\.id) { thread in
...
struct ArchiveThreadContentList_Previews: PreviewProvider {
    static var previews: some View {
        let appState = AppState.shared
        let vm = ThreadsViewModel()
        ArchivedThreadContentList(viewModel: vm)
