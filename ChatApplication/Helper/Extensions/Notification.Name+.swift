//
//  Notification.Name+.swift
//  ChatApplication
//
//  Created by hamed on 2/27/23.
//

import Foundation
extension Notification.Name {
    static let fileDeletedFromCacheName = Notification.Name("fileDeletedFromCacheName")
    static let connectName = Notification.Name("connectName")
    static let messageNotificationName = Notification.Name("messageNotificationName")
    static let systemMessageEventNotificationName = Notification.Name("systemMessageEventNotificationName")
    static let threadEventNotificationName = Notification.Name("threadEventNotificationName")
    static let callEventName = Notification.Name("callEventName")
    static let loginName = Notification.Name("LoginName")
}
