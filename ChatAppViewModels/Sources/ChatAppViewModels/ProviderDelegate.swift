...
public class ProviderDelegate: NSObject {
    ...
    public func reportIncomingCall(uuid: UUID, handle: String, hasVideo: Bool, completion: ((Error?) -> Void)? = nil) {
...
//        configureAudioSession()
        ...
        if let receiveCall = callState.call {
            ChatManager.call?.acceptCall(.init(callId: receiveCall.callId, client: .init(mute: !callState.answerType.mute, video: callState.answerType.video)), uniqueIdResult: nil)
        }
